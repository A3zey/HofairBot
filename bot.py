...
async def build_data_source_with_retry():
    """
    يحاول تجهيز مصدر البيانات، وإذا فشل (مثلاً تجاوز حد الطلبات اليومي)
    يعيد المحاولة كل فترة بدل ما يطيح البوت بالكامل من أول خطأ.
    """
    delay = 60
    while True:
        try:
            return build_data_source(config.WATCHLIST)
        except Exception as e:
            print(f"[!] تعذر تجهيز مصدر البيانات: {e}")
            print(f"    بنعيد المحاولة بعد {delay} ثانية...")
            await asyncio.sleep(delay)
            delay = min(delay * 2, 1800)  # تصاعدي لين حد أقصى 30 دقيقة


async def main_loop():
    bot = Bot(token=config.TELEGRAM_BOT_TOKEN)
    data_source = await build_data_source_with_retry()
    ...
    while True:
        ...
        for symbol in config.WATCHLIST:
            try:
                ...
                # خطأ مؤقت (مثلاً تجاوز حد الطلبات) - نتخطى هذا الرمز
                # هذي الدورة ونكمل الباقي، بدل ما نوقف البوت بالكامل
                print(f"[!] تعذر جلب سعر {symbol}: {e}")
                continue
